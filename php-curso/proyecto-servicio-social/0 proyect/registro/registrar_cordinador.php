...
if($_SERVER['REQUEST_METHOD']=='POST')
{
    ...
    $errores = ''; 
    if(empty($usuario) or empty($password) or empty($password2) or empty($carrera))
    {
     ...
     {  //hacer conexion con base de datos
        ...
        $statement = $conexion->prepare('SELECT * FROM Cordinador WHERE nombre = :usuario AND carrera = :carrera LIMIT 1');   //preparamos la sentencia con :usuario para cambiar el valor despues, limitamos a 1 para seguridad
       ...
       if($errores == '')
       {
           $statement = $conexion->prepare('INSERT INTO Cordinador VALUES(:usuario,:pass,:carrera)');
...
require 'views/registrar_cordinador_views.php';
?>